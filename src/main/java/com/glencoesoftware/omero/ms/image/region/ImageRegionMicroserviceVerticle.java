...
public class ImageRegionMicroserviceVerticle extends AbstractVerticle {
    ...
    private static final org.slf4j.Logger log = LoggerFactory.getLogger(ImageRegionMicroserviceVerticle.class);
    ...
    /**
     * Entry point method which starts the server event loop and initializes our
     * current OMERO.web session store.
     */
    @Override
    public void start(Future<Void> future) {
        ...
        ConfigStoreOptions store = new ConfigStoreOptions().setType("file").setFormat("yaml")
                .setConfig(new JsonObject().put("path", "conf/config.yaml")).setOptional(true);
        ConfigRetriever retriever = ConfigRetriever.create(vertx,
                new ConfigRetrieverOptions().setIncludeDefaultStores(true).addStore(store));
    ...
    public void deploy(JsonObject config, Future<Void> future) {
        ...
        if (omeroServer == null) {
            throw new IllegalArgumentException("'omero.server' block missing from configuration");
        }
        omeroServer.forEach(entry -> {
            System.setProperty(entry.getKey(), (String) entry.getValue());
        });

        context = new ClassPathXmlApplicationContext("classpath:ome/config.xml", "classpath:ome/services/datalayer.xml",
                "classpath*:beanRefContext.xml");
        preferences = (PreferenceContext) this.context.getBean("preferenceContext");
        ...
        vertx.deployVerticle(new RedisCacheVerticle(), new DeploymentOptions().setConfig(config));
        vertx.deployVerticle(new ImageRegionVerticle(omero.getString("host"), omero.getInteger("port"), context),
                new DeploymentOptions().setWorker(true).setMultiThreaded(true).setConfig(config));
        vertx.deployVerticle(new ShapeMaskVerticle(omero.getString("host"), omero.getInteger("port")),
                new DeploymentOptions().setWorker(true).setMultiThreaded(true).setConfig(config));

        HttpServerOptions options = new HttpServerOptions();
        options.setMaxInitialLineLength(
                config.getInteger("max-initial-line-length", HttpServerOptions.DEFAULT_MAX_INITIAL_LINE_LENGTH));
        options.setMaxHeaderSize(config.getInteger("max-header-size", HttpServerOptions.DEFAULT_MAX_HEADER_SIZE));
        options.setMaxChunkSize(config.getInteger("max-chunk-size", HttpServerOptions.DEFAULT_MAX_CHUNK_SIZE));
        ...
        Tracing tracing = Tracing.newBuilder()
                ...
                .sampler(Sampler.ALWAYS_SAMPLE).localServiceName("omero-ms-image-region").spanReporter(spanReporter)
                .build();
        ...
        router.route().order(-1) // applies before routes
                .handler(routingContextHandler).failureHandler(routingContextHandler);
        ...
        if (sessionStoreConfig == null) {
            throw new IllegalArgumentException("'session-store' block missing from configuration");
        }
        ...
        if (sessionStoreType.equals("redis")) {
            ...
            sessionStore = new OmeroWebJDBCSessionStore(sessionStoreUri, vertx);
        } else {
            throw new IllegalArgumentException("Missing/invalid value for 'session-store.type' in config");
        }
        router.route().handler(new OmeroWebSessionRequestHandler(config, sessionStore, vertx));

        // ImageRegion request handlers
        router.get("/webgateway/render_image_region/:imageId/:theZ/:theT*").handler(this::renderImageRegion);
        router.get("/webgateway/render_image/:imageId/:theZ/:theT*").handler(this::renderImageRegion);
        router.get("/webclient/render_image_region/:imageId/:theZ/:theT*").handler(this::renderImageRegion);
        router.get("/webclient/render_image/:imageId/:theZ/:theT*").handler(this::renderImageRegion);

        // ShapeMask request handlers
        router.get("/webgateway/render_shape_mask/:shapeId*").handler(this::renderShapeMask);
    ...
    /**
     * Get information about microservice. Confirms that this is a microservice
     *
    ...
    private void getMicroserviceDetails(RoutingContext event) {
        ...
        String version = Optional.ofNullable(this.getClass().getPackage().getImplementationVersion())
                .orElse("development");
        int maxTileLength = Integer.parseInt(Optional
                .ofNullable(preferences.getProperty("omero.pixeldata.max_tile_length")).orElse("1024").toLowerCase());
        JsonObject resData = new JsonObject().put("provider", "ImageRegionMicroservice").put("version", version)
                .put("features", new JsonArray().add("flip").add("mask-color").add("png-tiles"))
                .put("options", new JsonObject().put("maxTileLength", maxTileLength));
        if (!cacheControlHeader.equals("")) {
            resData.getJsonObject("options").put("cacheControl", cacheControlHeader);
        }
        event.response().putHeader("content-type", "application/json").end(resData.encodePrettily());
    }

    /**
     * Render image region event handler. Responds with an image body on success
     * based on the <code>imageId</code>, <code>z</code> and <code>t</code> encoded
     * in the URL or HTTP 404 if the {@link Image} does not exist or the user does
    ...
    private void renderImageRegion(RoutingContext event) {
        ...
        try {
            imageRegionCtx = new ImageRegionCtx(request.params(), omeroSessionKey, requestId);
        ...
        vertx.eventBus().<byte[]>send(ImageRegionVerticle.RENDER_IMAGE_REGION_EVENT, Json.encode(imageRegionCtx),
                result -> {
                    try {
                        if (result.failed()) {
                        ...
                        response.headers().set("Content-Length", String.valueOf(imageRegion.length));
                        if (!cacheControlHeader.equals("")) {
    ...
    /**
     * Render shape mask event handler. Responds with a <code>image/png</code> body
     * on success based on the <code>shapeId</code> encoded in the URL or HTTP 404
     * if the {@link Shape} does not exist or the user does not have permissions to
     * access it.
     *
     * @param event Current routing context.
     */
    private void renderShapeMask(RoutingContext event) {
        ...
        HttpServerRequest request = event.request();
        ShapeMaskCtx shapeMaskCtx = new ShapeMaskCtx(request.params(), event.get("omero.session_key"));

        final HttpServerResponse response = event.response();
        vertx.eventBus().<byte[]>send(ShapeMaskVerticle.RENDER_SHAPE_MASK_EVENT, Json.encode(shapeMaskCtx), result -> {
            try {
                if (result.failed()) {
                    Throwable t = result.cause();
                    int statusCode = 404;
                ...
                response.headers().set("Content-Length", String.valueOf(shapeMask.length));
